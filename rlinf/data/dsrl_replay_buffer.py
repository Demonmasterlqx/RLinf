...
def _field_manifest() -> dict[str, dict[str, object]]:
    return {
...
class CompactDSRLReplayBuffer:
    ...
    def __init__(
        ...
        *,
        ...
        self.bytes_per_transition = dsrl_replay_bytes_per_transition()
    ...
    @staticmethod
    def _flatten_trajectory(trajectory: Trajectory) -> dict[str, torch.Tensor]:
        ...
        for name in ("actions", "rewards", "terminations", "truncations"):
            ...
            shape, dtype = DSRL_REPLAY_FIELD_SPECS[name]
            expected_shape = (traj_len, batch_size, *shape)
            if not torch.is_tensor(value) or tuple(value.shape) != expected_shape:
                actual = tuple(value.shape) if hasattr(value, "shape") else None
                raise ValueError(
        ...
        if set(flat) != set(DSRL_REPLAY_FIELD_SPECS):
    ...
    def _append_flat(self, flat: Mapping[str, torch.Tensor]) -> None:
        ...
        for name in DSRL_REPLAY_FIELD_SPECS:
            value = flat[name]
    ...
    def _nested_batch(flat: Mapping[str, torch.Tensor]) -> dict[str, object]:
        return {
            "curr_obs": {
                "dsrl_images": flat["curr_obs.dsrl_images"],
                "states": flat["curr_obs.states"],
                "tactile_marker_motion": flat["curr_obs.tactile_marker_motion"],
            },
            "next_obs": {
                "dsrl_images": flat["next_obs.dsrl_images"],
                "states": flat["next_obs.states"],
                "tactile_marker_motion": flat["next_obs.tactile_marker_motion"],
            },
    ...
    def load_checkpoint(
        ...
        metadata = self.validate_checkpoint_metadata(
            load_path, expected_capacity=self.capacity_transitions
        )
        ...
        for shard_id in range(int(metadata["num_shards"])):
            ...
            if set(shard) != set(DSRL_REPLAY_FIELD_SPECS):
                raise ValueError(
                    "Tabero DSRL compact replay shard fields mismatch: expected "
                    f"{sorted(DSRL_REPLAY_FIELD_SPECS)}, got {sorted(shard)}."
                )
