...
load_run_env() {
  ...
  while IFS='=' read -r key value; do
    ...
    seen["${key}"]=1
    case "${key}" in
      TABERO_TASK0_RUN_ID) TABERO_TASK0_RUN_ID="${value}" ;;
  ...
  [[ "${TABERO_TASK0_RUN_ID}" =~ ^[0-9]{8}_[0-9]{6}_(smoke|formal)$ ]] || die "invalid run.env run ID"
...
if [[ -n "${RESUME_DIR}" ]]; then
  ...
  TABERO_TASK0_RUN_ID="${TABERO_TASK0_RUN_ID:-$(date +%Y%m%d_%H%M%S)_${MODE}}"
  [[ "${TABERO_TASK0_RUN_ID}" =~ ^[0-9]{8}_[0-9]{6}_${MODE}$ ]] || \
    die "TABERO_TASK0_RUN_ID must match YYYYMMDD_HHMMSS_${MODE}"

  if [[ "${MODE}" == "smoke" ]]; then
    output_prefix="tabero_task0_firm_action_expert_lora_8gpu_capacity_smoke"
  else
    output_prefix="tabero_task0_firm_action_expert_lora_8gpu_50step"
  fi
  OUTPUT_DIR="${RESULTS_ROOT}/${output_prefix}_${TABERO_TASK0_RUN_ID}"
...
export TABERO_TASK0_RUN_ID WANDB_RUN_ID WANDB_RESUME
