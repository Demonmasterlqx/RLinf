...
CONFIG_NAME="tabero_firm_sft_dual_rank_t2_precision_20k"
...
DEFAULT_NORM_STATS="${WORKSPACE_ROOT}/results/tabero_firm_pi0_base_full_lora_fp32_50k_20260804_214510/norm_stats/norm_stats.json"
...
if [[ "${MODE}" == "resume" ]]; then
  ...
  [[ -f "${RESUME_DIR}/actor/data_state.json" ]] || \
    die "resume data state is missing: ${RESUME_DIR}"
  [[ -f "${RESUME_DIR}/actor/model_state_dict/trainable_weights.pt" ]] || \
    die "resume trainable sidecar is missing: ${RESUME_DIR}"
  ...
  RUN_STAMP="${TABERO_FIRM_20K_RUN_STAMP:-$(date +'%Y%m%d_%H%M%S')}"
  if [[ "${MODE}" == "preflight" ]]; then
    RUN_NAME="tabero_firm_pi0_base_rlinf_dual_rank_bf16_20k_preflight_${RUN_STAMP}"
  else
...
export TABERO_FIRM_20K_RUN_NAME="${RUN_NAME}"
export TABERO_FIRM_20K_RUN_DIR="${RUN_DIR}"
export TABERO_FIRM_20K_CHECKPOINT_ROOT="${RUN_DIR}/checkpoints"
...
mkdir -p "${NORM_DIR}" "${TABERO_FIRM_20K_CHECKPOINT_ROOT}"
if [[ "${MODE}" == "resume" ]]; then
  ...
  EXISTING_NORM_STATS="${TABERO_FIRM_EXISTING_NORM_STATS:-${DEFAULT_NORM_STATS}}"
  [[ -f "${EXISTING_NORM_STATS}" ]] || \
    die "Firm norm stats do not exist: ${EXISTING_NORM_STATS}"
  cp -- "${EXISTING_NORM_STATS}" "${TABERO_FIRM_NORM_STATS}"
...

{
  ...
  printf 'global_batch_size=28\n'
  printf 'micro_batch_size=1\n'
  printf 'gradient_accumulation=4\n'
  printf 'paligemma_lora_rank=16\n'
  printf 'action_expert_lora_rank=32\n'
  printf 'master_precision=fp32\n'
  printf 'compute_precision=bf16_amp\n'
  printf 'fsdp_param_reduce_buffer_precision=fp32\n'
...
if [[ "${MODE}" == "preflight" ]]; then
  ...
  STEP2_DIR="${TABERO_FIRM_20K_CHECKPOINT_ROOT}/global_step_2"
  ...
  run_training train_stage2_resume.log \
    ...
    actor.optim.lr_decay_steps=4 \
    actor.fsdp_config.trainable_checkpoint_metadata.target_global_step=4
  STEP4_DIR="${TABERO_FIRM_20K_CHECKPOINT_ROOT}/global_step_4"
  ...
  FINAL_STEP_DIR="${TABERO_FIRM_20K_CHECKPOINT_ROOT}/global_step_20000"
